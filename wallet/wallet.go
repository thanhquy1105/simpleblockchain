...
func NewWallet() *Wallet {
	...
	digest2 := h2.Sum(nil)
	// 3. Perform SHA-256 hashing on the result of SHA-256 (20 bytes)
	h3 := sha256.New()
	h3.Write(digest2)
	digest3 := h3.Sum(nil)
	// 4. Add version byte in front of SHA-256 hash (0x00 for Main Network)
	vd4 := make([]byte, 21)
	vd4[0] = 0x00
	copy(vd4[1:], digest3[:])
	// 5. Perform SHA-256 hash on the extended SHA-256 result
	h5 := sha256.New()
	h5.Write(vd4)
	digest5 := h5.Sum(nil)
	...
	chsum := digest6[:4]
	// 8. Add the 4 checksum bytes from 7 at the end of extended SHA-256 hash from 4 (25 bytes)
	dc8 := make([]byte, 25)
	copy(dc8[:21], vd4[:])
	copy(dc8[21:], chsum[:])
